...
test("PRD-041 denies every unmocked public network mechanism", () => {
  assert.throws(() => globalThis.fetch("https://api.getcuna.com"), TypeError);
  ...
  assert.deepEqual(mechanisms, globalThis.WebSocket === undefined
    ? ["global.fetch", "global.fetch", "node:http.request", "node:https.request"]
    : [
      "global.fetch", "global.fetch", "node:http.request", "node:https.request", "WebSocket",
    ]);
});
