...
for (let run = 0; run < 2; run += 1) {
  ...
  try {
    ...
    const installed = JSON.parse(await readFile(path.join(workspace, "node_modules/@cuna_labs/sdk/package.json"), "utf8"));
    assert.equal(installed.name, "@cuna_labs/sdk");
...
const sbom = {
  ...
  metadata: {
    component: {
      "bom-ref": `pkg:npm/%40cuna_labs/sdk@${candidate.version}`,
      type: "library",
      name: "@cuna_labs/sdk",
      version: candidate.version,
      purl: `pkg:npm/%40cuna_labs/sdk@${candidate.version}`,
      hashes: [{ alg: "SHA-256", content: candidate.sha256 }],
    },
  },
  components: [],
  dependencies: [{
    ref: `pkg:npm/%40cuna_labs/sdk@${candidate.version}`,
    dependsOn: [],
  }],
};
